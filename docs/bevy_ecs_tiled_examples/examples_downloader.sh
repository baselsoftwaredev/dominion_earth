#!/usr/bin/env bash
# Download only the `examples` directory from adrien-bon/bevy_ecs_tiled@main
# into a plain directory (no .git, no history). No GitHub auth token required.
#
# Usage:
#   ./examples_downloader.sh [OUTDIR]
# Example:
#   ./examples_downloader.sh bevy_ecs_tiled-examples
#
...
OUTDIR="${1:-bevy_ecs_tiled-examples}"
OWNER="adrien-bon"
REPO="bevy_ecs_tiled"
REF="main"
TMPDIR="$(mktemp -d)"
