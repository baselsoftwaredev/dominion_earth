#!/usr/bin/env bash
# Download the `book` directory from adrien-bon/bevy_ecs_tiled@main
# into a plain directory (no .git, no history). No GitHub auth token required.
#
# Usage:
#   ./book_downloader.sh [OUTDIR]
# Example:
#   ./book_downloader.sh bevy_ecs_tiled-book
#
# This script downloads a repository archive (zip or tar.gz) for the given ref,
# extracts it to a temporary directory, and copies the `book/` tree into OUTDIR.
# Requirements: curl and either unzip or tar + cp (most systems have these).
set -euo pipefail

OUTDIR="${1:-bevy_ecs_tiled-book}"
OWNER="adrien-bon"
REPO="bevy_ecs_tiled"
REF="main"
TMPDIR="$(mktemp -d)"
...
SRC_DIR="$EXTRACTED_ROOT/book"
if [ ! -d "$SRC_DIR" ]; then
  echo "No book directory found in the extracted archive at: $SRC_DIR" >&2
  exit 1
fi

# Copy book to OUTDIR (preserve structure). Use rsync if available, otherwise cp -a.
mkdir -p "$OUTDIR"
...
echo "Done. book/ from ${OWNER}/${REPO}@${REF} saved to: $OUTDIR"
