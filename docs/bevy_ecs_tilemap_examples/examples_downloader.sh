#!/usr/bin/env bash
# Download only the `examples` directory from StarArawn/bevy_ecs_tilemap
# into a plain directory (no .git, no history). No GitHub auth token required.
#
# Usage:
#   ./examples_downloader.sh [OUTDIR] [REF]
# Examples:
#   ./examples_downloader.sh bevy_ecs_tilemap-examples
#   ./examples_downloader.sh bevy_ecs_tilemap-examples main
#
...
OUTDIR="${1:-bevy_ecs_tilemap-examples}"
OWNER="StarArawn"
...
if [ -z "$ARCHIVE_TYPE" ]; then
  ...
  echo "Please check that the ref exists (try 'main', 'latest', or a specific tag/branch)." >&2
  echo "Visit: https://github.com/${OWNER}/${REPO}" >&2
  exit 1
fi
...
echo "Done. examples/ from ${OWNER}/${REPO}@${REF} saved to: $OUTDIR"