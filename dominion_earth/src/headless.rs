...
pub fn run_headless_simulation(seed: Option<u64>) {
    ...
    for turn in 1..=target_turns {
        ...
        let execution_results =
            ai_coordinator.execute_decisions(&ai_decisions, &mut modified_state);
        ...
        performance_metrics.record_turn(
            turn,
            turn_duration,
            ai_decisions.len(),
    ...
    println!(
        ...
        total_duration.as_millis() as f32 / target_turns as f32
    );
    println!(
        "Target met: {}",
    ...
    println!(
        "Final state: Turn {}, {} civilizations",
        game_state.turn,
        game_state.civilizations.len()
    );
...
impl PerformanceMetrics {
    ...
    fn print_summary(&self) {
        ...
        let avg_decisions = self.ai_decision_counts.iter().sum::<usize>() as f32
            / self.ai_decision_counts.len() as f32;
        let avg_executions =
            self.execution_counts.iter().sum::<usize>() as f32 / self.execution_counts.len() as f32;

        println!("\n=== Performance Metrics ===");
        println!(
            "Turn time - Avg: {:?}, Min: {:?}, Max: {:?}",
        ...
        println!(
            "Performance target (200 turns < 2s, avg < 10ms): {}",
...
fn print_final_state(game_state: &GameState) {
    ...
    let mut scored_civs: Vec<_> = game_state
        .civilizations
        .iter()
        .map(|(id, data)| {
    ...
    // Economic statistics
    let total_gold: f32 = game_state
        .civilizations
        .values()
        .map(|data| data.civilization.economy.gold)
        .sum();

    let total_military: f32 = game_state
        .civilizations
        .values()
