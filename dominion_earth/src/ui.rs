...
use core_sim::{Position, Civilization, resources::{CurrentTurn, WorldMap}};
...
pub fn ui_system(
    ...
    egui::SidePanel::left("game_panel")
        ...
        .show(ctx, |ui| {
            ...
            ui.label(format!("Status: {}", if game_state.paused { "Paused" } else { "Running" }));
            ui.label(format!("Auto-advance: {}", if game_state.auto_advance { "On" } else { "Off" }));
            ui.separator();
            ...
            if response.hovered() {
                ...
                for (_i, civ) in civs_vec.iter().enumerate().take(10) {
                    if let Some(capital) = civ.capital {
                        ...
                        let point = rect.min + egui::Vec2::new(
                            x_ratio * rect.width(),
                            y_ratio * rect.height()
                        );
                        
                        painter.circle_filled(point, 3.0, egui::Color32::from_rgb(
